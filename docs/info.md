...
The only source of the memory will come from `ui_in` ports directly. When `write_en` goes high, the current element (`in_data`) connected to all bits at the dedicated input `ui_in` is loaded into memory at its specified byte address, on the next rising edge. 
